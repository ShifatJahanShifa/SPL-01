...
void writeNum(int fd, uint16_t n) {
    uint8_t bytes[2];
    bytes[0] = n & 0xFF;
    bytes[1] = n >> 8;
    write(fd, bytes, 2);
...
GIF *newGif(const char *fileName, uint16_t width, uint16_t height,
    uint8_t *palette, int depth, int backgroundIndex, int loop)
{
    ...
    GIF *gif =(GIF*) calloc(1, sizeof(*gif) + numberOFbuffers*width*height);
    ...
    uint8_t buffer[3];
    buffer[0] = 0xF0 | (depth - 1);
    buffer[1] = (uint8_t) backgroundIndex;
    buffer[2] = 0x00;
    write(gif->fileDescriptor, buffer, 3);
        ...
        for (red = 0; red < 6; red++)
        {
            for (green = 0; green < 6; green++)
            {
                for (blue = 0; blue < 6; blue++)
                {
                    ...
                    writeAndStore(storeGct, palette, gif->fileDescriptor, buffer, 3);
                    if (++i == 1 << depth)
                        goto doneGct;
...
static void putLoop(GIF *gif, uint16_t loop)
{
    uint8_t buffer[3] = {'!', 0xFF, 0x0B};
    write(gif->fileDescriptor, buffer, 3);

    write(gif->fileDescriptor, "NETSCAPE2.0", 11);   // escaping

    uint8_t buffer1[2] = {0x03, 0x01};
    write(gif->fileDescriptor, buffer1, 2);
...
static void endKey(GIF *gif)
{
    ...
    if (byteOffset)
    {
        uint8_t buffer[1] = {byteOffset};
        write(gif->fileDescriptor, buffer, 1);
...
static void putImage(GIF *gif, uint16_t w, uint16_t h, uint16_t x, uint16_t y)
{
    ...
    uint8_t buffer[2] = {0x00, gif->depth};
    write(gif->fileDescriptor, buffer, 2);
...
static void addGraphicsControlExtension(GIF *gif, uint16_t delay)
{
   ...
   // 4 byte block
    uint8_t buffer[4] = {'!', 0xF9, 0x04, flags};
    write(gif->fileDescriptor, buffer, 4);
    ...
    uint8_t buffer1[2] = {(uint8_t) gif->backgroundIndex, 0x00};
    write(gif->fileDescriptor, buffer1, 2);
